...
from happysimulator import (
    ...
    Simulation, SimulationSummary, Source,
)
from happysimulator.components.industrial import (
    RenegingQueuedResource, Shift, ShiftSchedule,
)
...
class IVRMenu(QueuedResource):
    ...
    def handle_queued_event(self, event: Event) -> Generator[float, None, list[Event]]:
        ...
        return [
            Event(time=self.now, event_type="Call",
                  target=self.router, context=ctx)
        ]
...
class AgentPool(RenegingQueuedResource):
    ...
    def __init__(self, name: str, service_time: float, num_agents: int,
                 downstream: Entity, reneged_target: Entity):
        super().__init__(
            name, reneged_target=reneged_target,
            default_patience_s=300.0, policy=FIFOQueue(),
        )
...
def run_call_center_simulation(config: CallCenterConfig | None = None) -> CallCenterResult:
    ...
    # Agent pools
    pools = {
        "sales": AgentPool("Sales", config.sales_service,
                           config.morning_agents, sink, abandoned),
        "support": AgentPool("Support", config.support_service,
                             config.morning_agents, sink, abandoned),
        "billing": AgentPool("Billing", config.billing_service,
                             config.morning_agents, sink, abandoned),
    }
    ...
    source = Source.poisson(
        rate=config.arrival_rate, target=ivr,
        event_type="IncomingCall", name="Calls",
    ...
    return CallCenterResult(
        sink=sink, abandoned=abandoned, ivr=ivr,
        pools=pools, config=config, summary=summary,
    )
...
def print_summary(result: CallCenterResult) -> None:
    ...
    for name, pool in result.pools.items():
        ...
        print(f"  {name.title()}: handled={pool.calls_handled}, "
