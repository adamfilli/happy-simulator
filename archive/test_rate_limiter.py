...
def test_immediate_forwarding():
    ...
    client = Client("c")
    server = Server("s", server_latency=ConstantLatency(Instant.Epoch))

    req = Request(time=Instant.Epoch, client=client, server=server, callback=rl.start_request)
...
def test_refill_and_multiple_requests():
    ...
    req1 = Request(time=Instant.Epoch, client=client, server=server, callback=rl.start_request)
