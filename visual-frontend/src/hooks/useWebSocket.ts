...
export function useWebSocket() {
  ...
  const { setState, addEvents, addEdges, addLogs, setPlaying, setEdgeStats, setBreakpointHitInfo } = useSimStore();
  ...
  useEffect(() => {
    ...
    ws.onmessage = (e) => {
      ...
      if (msg.type === "state_update") {
        ...
        setPlaying(false);
        if (msg.breakpoints) {
          setBreakpointHitInfo(msg.breakpoints);
        }
      }
    };
  ...
  }, [setState, addEvents, addEdges, addLogs, setPlaying, setEdgeStats, setBreakpointHitInfo]);
