...
class TestLRUAlgorithmComparison:
    ...
    def test_scan_resistance_comparison(self, test_output_dir):
        ...
        for policy_name, hr_list in hit_rate_over_time.items():
            ...
            ax2.plot(x_points[:len(hr_list)], [r * 100 for r in hr_list],
                    label=policy_name, linewidth=2, color=colors[color_idx])
...
"""
    ...
    def test_temporal_locality_comparison(self, test_output_dir):
        ...
        for bar, count in zip(bars2, evictions):
            ax2.annotate(f'{count}',
                        xy=(bar.get_x() + bar.get_width() / 2, count + 10),
                        ha='center', va='bottom', fontsize=9)

        plt.suptitle(f'Temporal Locality Workload (Cache={cache_size}, Keys={num_keys})', fontsize=14)
    ...
    def test_working_set_shift_comparison(self, test_output_dir):
        ...
        bars = ax2.bar(policy_names, [adaptation_scores[p] * 100 for p in policy_names],
    ...
    def test_sample_size_impact(self, test_output_dir):
        ...
        ax1.plot(sample_sizes, [r * 100 for r in sampled_hit_rates], 'bo-', linewidth=2, markersize=8)
        ax1.axhline(y=lru_hit_rate * 100, color='red', linestyle='--', linewidth=2, label=f'True LRU ({lru_hit_rate*100:.1f}%)')
        ...
        ax2.bar(range(len(sample_sizes)), accuracy, tick_label=[str(s) for s in sample_sizes],
        ...
        for i, (size, acc) in enumerate(zip(sample_sizes, accuracy)):
            if size <= 10:
                savings = (1 - size / cache_size) * 100
                ax2.annotate(f'{acc:.0f}%\n({savings:.0f}% mem saved)',
                            xy=(i, acc + 2), ha='center', fontsize=8)
    ...
    def test_slru_segment_ratio_impact(self, test_output_dir):
        ...
        ax1.plot(protected_ratios, [r * 100 for r in scan_hit_rates], 'b-o',
                linewidth=2, markersize=8, label='Scan + Hot Workload')
        ax1.plot(protected_ratios, [r * 100 for r in random_hit_rates], 'g-s',
                linewidth=2, markersize=8, label='Frequency Skew Workload')
        ...
        best_random_idx = random_hit_rates.index(max(random_hit_rates))
        ax1.annotate(f'Best: {protected_ratios[best_scan_idx]}',
        ...
        ax2.stackplot(protected_ratios,
                     [[r * 100 for r in protected_ratios],
                      [(1 - r) * 100 for r in protected_ratios]],
    ...
    def _scan_then_hot_workload(self, num_keys: int, hot_set_size: int, num_accesses: int) -> List[str]:
        ...
        for _ in range(num_accesses - num_keys):
