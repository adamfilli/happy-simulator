...
class TickProvider(EventProvider):
    """Generates tick events for a counter."""

    def __init__(self, counter: TickCounter):
        super().__init__()
        self.counter = counter

    def get_events(self, time: Instant) -> List[Event]:
        return [Event(time=time, event_type="Tick", target=self.counter)]
...
class TestDecoratedCounterSimulation:
    ...
    def test_counter_receives_events_from_source(self):
        ...
        profile = ConstantRateProfile(rate=10.0)  # 10 events/second
        provider = TickProvider(counter)
        arrival = ConstantArrivalTimeProvider(profile, start_time=Instant.Epoch)
        source = Source("ticker", provider, arrival)
    ...
    def test_counter_timestamps_are_monotonic(self):
        ...
        profile = ConstantRateProfile(rate=5.0)
        provider = TickProvider(counter)
        arrival = ConstantArrivalTimeProvider(profile, start_time=Instant.Epoch)
        source = Source("ticker", provider, arrival)
