...
fifo_server = QueuedServer(name="FifoServer",
                      ...
                      queue=fifo_queue)
fifo_request_generator = Source(func=lambda time: [Request(time=time, client=fifo_client, server=fifo_server, callback=fifo_client.send_request, network_latency=network_latency)],
...
lifo_server = QueuedServer(name="LifoServer",
                      ...
                      threads=server_threads,
                      queue=lifo_queue)
lifo_request_generator = Source(func=lambda time: [Request(time=time, client=lifo_client, server=lifo_server, callback=lifo_client.send_request, network_latency=network_latency)],
                              arrival_rate_profile=profile,
                              distribution=arrival_distribution)
