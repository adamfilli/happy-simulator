...
class RequestEvent(Event):
    """A request event targeting a ConcurrencyTrackerEntity."""
...
class RequestProvider(EventProvider):
    """Provides RequestEvents targeting a ConcurrencyTrackerEntity."""
    
    def __init__(self, tracker: ConcurrencyTrackerEntity):
        super().__init__()
        self.tracker = tracker
    
    def get_events(self, time: Instant) -> List[Event]:
        return [RequestEvent(time, self.tracker)]
...
def test_increasing_concurrency_under_overload(test_output_dir: Path):
    ...
    provider = RequestProvider(tracker)
