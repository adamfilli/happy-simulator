...
class TestCacheHitRateVisualization:
    ...
    def test_hit_rate_vs_cache_size(self, test_output_dir):
        ...
        best_efficiency_idx = np.argmax([hr / (cs / num_keys) for hr, cs in zip(hit_rates, cache_sizes)])
        ax2.annotate(
...
class TestEvictionPolicyComparison:
    ...
    def test_eviction_policies_comparison(self, test_output_dir):
        ...
        plt.suptitle(f'Eviction Policy Comparison (Cache Size={cache_size}, Keys={num_keys})', fontsize=14)
        plt.tight_layout()
...
class TestCacheWarmupVisualization:
    ...
    def test_cold_start_warmup(self, test_output_dir):
        ...
        for i, key in enumerate(keys):
            ...
            was_hit = cache.stats.hits > (sum(1 for _ in range(i)) - cache.stats.misses + cache.stats.hits - 1) if i > 0 else False
        ...
        ax1.grid(True, alpha=0.3)
        ax1.axhline(y=cumulative_hit_rates[-1] * 100, color='r', linestyle='--',
                   label=f'Final: {cumulative_hit_rates[-1]*100:.1f}%')
        ax1.legend()

        # Cache size utilization
        ax2.axhline(y=cache_size, color='r', linestyle='--', label='Capacity')
        ax2.axhline(y=cache2.cache_size, color='b', linestyle='-',
                   label=f'Final Size: {cache2.cache_size}')
        ax2.set_xlabel('Time')
...
class TestWritePolicyVisualization:
    ...
    def test_write_through_vs_back(self, test_output_dir):
        ...
        total_wt = 0
        total_wb = 0
        for wt, wb in zip(write_through_latencies, write_back_latencies):
            total_wt += wt
            total_wb += wb
            wt_cumulative.append(total_wt)
            wb_cumulative.append(total_wb)

        ax1.plot(range(1, num_writes + 1), [t * 1000 for t in wt_cumulative],
                'b-', linewidth=2, label='Write-Through')
        ax1.plot(range(1, num_writes + 1), [t * 1000 for t in wb_cumulative],
                'g-', linewidth=2, label='Write-Back')
        ...
        # Per-write latency
        ax2.bar(['Write-Through', 'Write-Back'],
