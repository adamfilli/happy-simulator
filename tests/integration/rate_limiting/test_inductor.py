...
def _profile_line(profile: Profile, end_s: float, n_points: int = 300) -> tuple[list[float], list[float]]:
...
_PROFILES: dict[str, tuple[Profile, float]] = {
    ...
    "linear_ramp_5_to_50": (LinearRampProfile(duration_s=60.0, start_rate=5.0, end_rate=50.0), 60.0),
    "step_10_to_50": (StepRateProfile(low=10.0, high=50.0, step_time=20.0), 60.0),
    "periodic_burst": (PeriodicBurstProfile(base_rate=10.0, burst_rate=80.0, period=10.0, burst_duration=2.0), 60.0),
}
...
def test_inductor_burst_suppression(profile_name: str, test_output_dir: Path):
    ...
    _save_csv(test_output_dir / f"inductor_{profile_name}_output.csv", {"time": out_t, "rate": out_r})
...
def test_inductor_vs_rate_limiters_comparison(test_output_dir: Path):
    ...
    tb = RateLimitedEntity(
        "token_bucket", downstream=sink_tb,
    ...
    lb = RateLimitedEntity(
        "leaky_bucket", downstream=sink_lb,
    ...
    for ax, (label, entity) in zip(axes, configs):
        ...
        stats = entity.stats
        ax.set_ylabel("Rate (events/s)")
        ax.set_title(f"{label}  [fwd={stats.forwarded}, drop={stats.dropped}, q={entity.queue_depth}]")
        ax.legend(loc="upper right")
        ax.grid(True, alpha=0.3)
