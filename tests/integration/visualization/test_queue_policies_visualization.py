...
class TestAdaptiveLIFOVisualization:
    ...
    def test_mode_switching_under_load(self, test_output_dir):
        ...
        # Mode indicator
        fifo_times = [t for t, m in zip(times, modes, strict=False) if m == "FIFO"]
        [d for d, m in zip(depths, modes, strict=False) if m == "FIFO"]
        lifo_times = [t for t, m in zip(times, modes, strict=False) if m == "LIFO"]
        [d for d, m in zip(depths, modes, strict=False) if m == "LIFO"]
...
class TestWeightedFairQueueVisualization:
    ...
    def test_weighted_distribution(self, test_output_dir):
        ...
        # Ratio over time
        ratios = []
        for p, s in zip(premium_counts[1:], standard_counts[1:], strict=False):
            if s > 0:
                ratios.append(p / s)
            else:
                ratios.append(0)
...
class TestDeadlineQueueVisualization:
    ...
    def test_deadline_ordering_and_expiration(self, test_output_dir):
        ...
        ax.barh(range(len(items)), deadlines, color=colors, alpha=0.7)
...
class TestQueuePolicyComparison:
    ...
    def test_fifo_vs_lifo_latency(self, test_output_dir):
        ...
        # Cumulative latency
        ax2.plot(
            sorted(fifo_latencies),
            np.linspace(0, 1, len(fifo_latencies)),
            "b-",
            label="FIFO",
            linewidth=2,
        )
        ax2.plot(
            sorted(adaptive_latencies),
            np.linspace(0, 1, len(adaptive_latencies)),
            "g-",
            label="AdaptiveLIFO",
            linewidth=2,
        )
        ...
        sorted(fifo_latencies)[len(fifo_latencies) // 2]
        sorted(adaptive_latencies)[len(adaptive_latencies) // 2]
