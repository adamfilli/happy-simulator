...
class PipelineEventProvider(EventProvider):
    """Provides events that enter the pipeline at FirstEntity."""
    
    def __init__(self, first_entity: FirstEntity):
        super().__init__()
...
def test_tracing_spans_through_pipeline():
    ...
    provider = PipelineEventProvider(first)
...
def test_tracing_captures_errors():
    ...
    class FailingEventProvider(EventProvider):
        def __init__(self, target: Entity):
            super().__init__()
            self.target = target
        
        def get_events(self, time: Instant) -> List[Event]:
            return [Event(time=time, event_type="FailEvent", target=self.target)]
    ...
    provider = FailingEventProvider(failing_entity)
    arrival_time_provider = ConstantArrivalTimeProvider(profile, Instant.Epoch)
