"""Adverse Advertising Amplification v2 — Agent-based simulation.

Unlike v1 (aggregate tier model), this version models 10,000 individual
consumers who visit a platform, generating click and conversion events
through a probabilistic ad funnel. The AAA effect emerges from individual
behavior rather than aggregate math.

## Architecture

- **ConsumerRecord** (dataclass): 10,000 consumers with segment, sentiment,
  visit rate, and saturation. NOT entities — pure data.
- **Platform** (Entity): Receives visits, runs ad funnel (exposure → click →
  conversion), emits AdClick/Sale events.
- **SellerEntity** (Entity): Tracks per-segment profit, weekly evaluation
  deactivates unprofitable segments.
- **AdvertiserEntity** (Entity): Platform revenue collector, manages active
  segment set.
...
# Segment definitions
# CPA = CPC / (conv_rate * eff_sentiment).  Breakeven when CPA >= margin.
# Conv_rate is P(convert | click), modulated by sentiment.
# Click rate is fixed (not affected by sentiment).
SEGMENTS = {
...
# Aggregate visit rate: each consumer ~0.0208 visits/hr ≈ 3.5 visits/week
...
# =============================================================================
# Consumer pool (pure data)
# =============================================================================


@dataclass
class ConsumerRecord:
    """A single consumer in the pool. NOT an Entity."""

    id: int
    segment: str
    base_sentiment: float  # individual baseline ~N(0.85, 0.10), clipped
    visit_rate: float  # visits per hour
    saturation: float = 0.0  # ad fatigue, increases with exposure


def _build_consumer_pool(rng: random.Random) -> list[ConsumerRecord]:
    """Create 10,000 consumers with segment-weighted distribution."""
        ...
        for _ in range(seg_info["size"]):
            ...
            consumers.append(
                ConsumerRecord(
                    ...
                    visit_rate=VISIT_RATE_PER_CONSUMER,
                )
            )
            cid += 1
    return consumers
...
# =============================================================================
# Smooth sentiment curve (reused from v1)
# =============================================================================
...
class SellerEntity(Entity):
    ...
    def __init__(self, name: str, advertiser: AdvertiserEntity):
        ...
        # Per-segment accumulators (reset each evaluation period)
        ...
        self.ad_spend_data = Data()  # total ad spend per week
...
class Platform(Entity):
    """Ad-serving platform. Receives Visit events, runs probabilistic funnel.

    For each visit:
    1. Pick a random consumer from the pool
    2. Check if consumer's segment is active (targeted by seller)
    3. Roll exposure → click → conversion probabilities
    4. Emit AdClick to advertiser + AdCost to seller on click
    5. Emit Sale to seller on conversion
    """
    ...
    def __init__(
        ...
        self._segment_consumers: dict[str, list[ConsumerRecord]] = {
            s: [] for s in SEGMENT_NAMES
        }
        for c in consumers:
            self._segment_consumers[c.segment].append(c)

        # Data for visualization
        self.sentiment_data = Data()  # global sentiment adjustor
        self.mean_sentiment_data = Data()  # mean effective sentiment

    def handle_event(self, event: Event) -> list[Event] | None:
        if event.event_type == "Visit":
            return self._handle_visit(event)
    ...
    def _handle_visit(self, event: Event) -> list[Event] | None:
        # Pick a random consumer
        consumer = self._rng.choice(self._consumers)

        # Decay saturation on every visit
        consumer.saturation = max(0.0, consumer.saturation - SATURATION_DECAY)

        # Check if segment is active
        if consumer.segment not in self._advertiser.active_segments:
            return None  # organic visit, no ad served

        seg_info = SEGMENTS[consumer.segment]

        # Effective sentiment for this consumer
        eff_sentiment = max(0.01, consumer.base_sentiment + self._global_sentiment_adj)

        # Saturation multiplier (reduces conversion effectiveness)
        sat_mult = 1.0 - consumer.saturation * 0.5

        # Exposure (always shown an ad if segment is active)
        consumer.saturation = min(1.0, consumer.saturation + SATURATION_INCREASE)

        # Click probability — NOT affected by sentiment (people still click)
        ...
        results: list[Event] = [
            Event(
                ...
                event_type="AdClick",
                target=self._advertiser,
                context={"segment": consumer.segment, "cpc": cpc},
            ),
            Event(
                time=event.time,
                event_type="AdCost",
...
@dataclass
class AAAv2Scenario:
    """A prepared v2 scenario (built but not yet run)."""
...
def build_scenario(seed: int = SEED) -> AAAv2Scenario:
    """Build the agent-based AAA simulation."""
    ...
    sim = Simulation(
        sources=[source],
        entities=[platform, advertiser, seller],
...
def visualize_timeline(
    ...
    # Convert adjustor to effective mean: base ~0.85 + adjustor
    mean_effective = [0.85 + v for v in sent_values]

    # Platform revenue: aggregate per evaluation period
    ...
    # Need tier_counts in week-space for regime backgrounds
...
def _bucket_advertiser_revenue(
    ...
    rev_samples = advertiser.revenue_data.values  # list of (time_s, cpc)
    buckets: list[float] = []
...
def _build_visual_charts(scenario: AAAv2Scenario) -> list:
...
def print_summary(scenario: AAAv2Scenario, summary) -> None:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Adverse Advertising Amplification v2 (Agent-Based)"
    )
    parser.add_argument(
        "--visual",
        action="store_true",
    ...
    if args.visual:
        ...
        print("  Launching visual debugger for AAA v2...")
        serve(scenario.sim, charts=charts)
        return

    print("=" * 74)
    print("  ADVERSE ADVERTISING AMPLIFICATION v2 (Agent-Based)")
    print("  10,000 individual consumers with probabilistic ad funnel")
    print("=" * 74)

    scenario = build_scenario()
    print(f"\n  Running simulation ({DURATION_WEEKS} weeks, {TOTAL_CONSUMERS:,} consumers)...")
