...
def test_immediate_forwarding():
    ...
    client = Client("c")
    server = Server("s", server_latency=ConstantLatency(Instant.from_seconds(0)))

    req = Request(time=Instant.from_seconds(0), client=client, server=server, callback=rl.start_request)
    ...
    assert ev.time == Instant.from_seconds(0)
...
def test_delayed_forwarding():
    ...
    assert ev.time == Instant.from_seconds(1)
    ...
    assert ev2.time == Instant.from_seconds(1)
...
def test_refill_and_multiple_requests():
    ...
    req1 = Request(time=Instant.from_seconds(0), client=client, server=server, callback=rl.start_request)
    ...
    req2 = Request(time=Instant.from_seconds(0.5), client=client, server=server, callback=rl.start_request)
    ...
    req3 = Request(time=Instant.from_seconds(2.0), client=client, server=server, callback=rl.start_request)
    evs3 = rl.start_request(req3)
    assert evs3[0].callback == server.start_request
    assert evs3[0].time == Instant.from_seconds(2.0)
